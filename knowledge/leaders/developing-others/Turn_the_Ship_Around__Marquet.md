...
# Source Summary — *Turn the Ship Around!*

`product: Group Leader eCoach · code: (RAGSRC) · owner: Ned Howey · review milestone: TBD · version: v1 · date: 2026-06-10`

**Source:** L. David Marquet — *Turn the Ship Around! A True Story of Turning Followers into Leaders* (Portfolio/Penguin, 2012; ~66.5K words).
**What it is:** A first-person account of how a submarine captain stopped running his crew top-down and built a "leader-leader" structure instead — divesting decision control while raising competence and clarity, so people led themselves. The book is organized as three pillars: **Control**, **Competence**, **Clarity**.

**Altitude reframe (applies to the whole document).** The nuclear submarine, its 135 ranked-and-paid sailors, and the Navy's command machinery are the *vehicle*, not the subject. Every idea here is retold as an illustration for a first-time volunteer group leader. The coach should never present Marquet as being "about" military command, and should swap the submarine specifics for the leader's own group when teaching.

**The authority inversion (read before tagging the Control entries).** Marquet's starting problem is the opposite of the volunteer leader's. He held *positional, rank-based control* and had to give it away. A first-time volunteer leader holds **earned, consent-based authority [A1.1]** — no rank, no compulsion — and yet *still* over-centralizes: does everything, becomes the bottleneck, can't let go. So the book's headline move ("divest control") does not transfer as a literal instruction. What transfers is the underlying claim: **centralization is a habit and a design choice, not just a function of formal power.** A leader with no power to compel can still hoard the thinking, the decisions, and the doing — and Marquet's mechanisms for releasing that grip work just as well, sometimes better, without rank behind them. The control entries carry `Source reach:` lines marking this.

**Sizing note.** This summary lands sub-band (~7.5K words ≈ ~11% of the source), below the 15–25% guideline, and that is deliberate. Three forces compress it: (1) the altitude reframe cuts the book's large narrative and Navy-technical mass; (2) the book is one big idea (the Control–Competence–Clarity triad) elaborated through ~21 "mechanisms," many of which are illustrations of the same few principles; (3) single-source-of-truth discipline — Marquet enters a well-supplied region of the corpus (autonomy, distributing leadership, culture), so overlapping concepts are written as *differentials* that defer to the source that already holds them rather than re-expanded here. See "Excluded" and the per-entry `Source reach:` and deferral notes.
...
**1. The default leadership model wastes people.** Marquet opens by arguing that the familiar leader-follower structure — one person directs, the rest comply — was optimized for extracting physical work and is badly suited to work that requires judgment. Treated as followers, capable people run at half-speed and stop offering their thinking. This sets up the search for a different structure.
concepts: [A1.2 enabling-vs-doing] [A3.2 ownership-and-voice]

**2. Empowerment is the wrong fix.** The standard remedy, empowerment programs, fails because the method contradicts the message: it takes the leader to "empower" people, which re-asserts their dependence. This sets up the need for a structural change, not a motivational program.
concepts: [A1.2 enabling-vs-doing] [A7.1 building-and-using-power]

**3. The bridge is control: push decision authority down.** Marquet's first move is to relocate decision rights to the people closest to the information and the work, while keeping responsibility himself. The signature practice is intent-based language ("I intend to…"). This raises an immediate problem: people newly handed decisions can make bad ones.
concepts: [A9.2 delegating-outcomes-not-tasks] [A3.2 ownership-and-voice] [A7.3 autonomy-vs-direction]

**4. Control requires competence.** Because a decision is only as sound as the knowledge behind it, divesting control forces a parallel investment in everyone's skill and understanding — learning as a constant practice, verifying rather than assuming understanding, building a shared habit of acting deliberately to catch errors. This is the first precondition.
concepts: [A6.6 learning-together] [A9.1 developing-leaders]

**5. Control requires clarity.** Decentralized decisions only stay aligned if everyone shares a clear picture of the group's purpose and the criteria for choosing well — otherwise autonomy fragments. Clarity is built through repeated message, guiding principles that function as decision criteria, and a culture that aims at excellence and welcomes questioning. This is the second precondition.
concepts: [A2.1 shared-purpose] [A2.3 values-as-glue] [A8.2 productive-dissent]

**6. The synthesis is emancipation, and it endures.** With control, competence, and clarity together, people are not "empowered" — they are released; the leader is no longer the source of their motivation. Marquet closes on the payoff: results that held for a decade after he left and a disproportionate crop of new leaders. The proof of leader-leader is what happens once the leader is gone.
concepts: [A1.2 enabling-vs-doing] [A9.4 succession-and-ceding-control] [A9.5 functional-shared-leadership]

---
## Layer 2 — Concept Entries

### "I intend to…": language that moves ownership down
concepts:    [A3.2 ownership-and-voice] [A9.2 delegating-outcomes-not-tasks]
...
- **Why it matters / evidence:** For a volunteer leader drowning in "what should I do?" messages, this is the cheapest possible intervention — it requires no authority, only a change in the question you'll accept. It directly attacks the bottleneck where everything routes through one person [A9.3]. Marquet credits the language shift as the practical engine of the whole turnaround.
- **Related concepts:** ownership-and-voice → emancipation [A1.2] (intent-language is how release happens in practice); delegating-outcomes [A9.2] (it hands over the decision, not just the task); resisting solutions [A1.2] (the leader must not answer the intention with an order).
- **Source reach:** Marquet's people had rank and a chain of command; a volunteer leader has neither, but the language transfers intact — arguably more easily, since there is no rank for members to hide behind.

### Divest control, keep responsibility
concepts:    [A9.2 delegating-outcomes-not-tasks] [A7.3 autonomy-vs-direction]
...
- **Why it matters / evidence:** The volunteer leader's most common complaint — *"I'm doing everything myself"* — is usually a control problem wearing a time-management costume. The fix is not better personal organization; it is relocating decisions. Marquet's whole case is that this is what produced both better results and durability.
- **Related concepts:** leader-leader [A9.5] (divesting is the mechanism of distributed leadership); avoiding-bottleneck [A9.3] (this is the direct cure); the C-C-C triad [A6.6] (don't divest faster than competence/clarity rise).
- **Source reach:** Marquet was giving away power he formally held; the volunteer leader is more often *failing to give away* a centralization they were never granted. The transferable claim is that hoarding decisions is a choice, and the cure is the same.
...
- **Why it matters / evidence:** Volunteer leaders often under-specify the goal and over-specify the method — the reverse of what helps. Getting it right turns "no one takes initiative" into people acting without being told, because they finally know the target and own the path.
- **Related concepts:** autonomy-vs-direction [A7.3] (this is the operating rule for the how/what split); intrinsic-motivation [A3.1] (autonomy on the how is a core driver — held by Pink); intent-based language [A3.2] (people announce *how* they'll meet the goal).
...
- **Why it matters / evidence:** First-time leaders default to being the helpful expert, not seeing that helpfulness can be disempowering. Resisting the urge is the daily practice behind every "develop your people" intention.
- **Related concepts:** enabling-vs-doing [A1.2] (the leader sets conditions instead of supplying output); coaching-members [A9.6] (questions over answers grows people); intent-based language [A3.2] (the member brings the solution, framed as intention).

### Eliminate top-down monitoring
concepts:    [A9.3 avoiding-founder-bottleneck] [A3.2 ownership-and-voice]
...
- **Why it matters / evidence:** Volunteer leaders often respond to flakiness by adding reminders, check-ins, and trackers — which can deepen the very dependence they're fighting. Marquet's counterintuitive move is to remove the scaffolding so responsibility has somewhere to land.
- **Related concepts:** avoiding-bottleneck [A9.3] (leader-owned tracking is a bottleneck); ownership-and-voice [A3.2] (self-reporting builds ownership); peer-accountability [A8.3] (where the responsibility moves when monitoring is removed).
- **Source reach:** Marquet could remove a monitoring system by command; a volunteer leader removes their own habit of chasing. Same message-effect, no authority required.

### Think out loud
concepts:    [A6.5 psychological-safety] [A8.2 productive-dissent]
...
- **Why it matters / evidence:** For a volunteer group, normalizing tentative, unfinished speech ("here's what I'm thinking, not sure yet…") is a low-cost way to surface dissent and prevent the silent drift into groupthink. It is psychological safety enacted as a habit, not a poster.
- **Related concepts:** psychological-safety [A6.5] (leader voicing doubt makes doubt safe); productive-dissent [A8.2] (spoken thinking surfaces disagreement early); trust-building [A4.3] (transparency of reasoning builds trust).

### When you step in, they step back
concepts:    [A1.2 enabling-vs-doing] [A9.1 developing-leaders]
situations:  [S:doing-everything-myself] [S:no-one-takes-initiative]
handoff:
anchors to:  Leader-leader, not leader-follower
source:      Marquet — *Turn the Ship Around!*, ch.14 ("A New Ship")
...
- **Why it matters / evidence:** Volunteer leaders rescue constantly — finishing a half-done task, jumping into a stalling conversation — and wonder why no one steps up. Often it's because stepping up keeps getting overridden. Restraint is the development tool.
- **Related concepts:** enabling-vs-doing [A1.2] (rescuing is reverting to doing); developing-leaders [A9.1] (letting people finish is how they grow); fear-of-delegating [A1.5] (the anxiety that drives the takeover).

### Control without competence is chaos (the first precondition)
...
- **Why it matters / evidence:** This entry answers the volunteer leader's most honest objection to delegating — *"if I let go, it'll fall apart."* Sometimes that fear is correct, and the answer isn't to keep control forever but to build competence deliberately so control can move. It reframes "I can't let go" as "I haven't yet built what would let me."
- **Related concepts:** divest-control [A9.2] (this is the precondition that makes divesting safe); developing-leaders [A9.1] (closing the competence gap is the work); fear-of-delegating [A1.5] (legitimate when competence is genuinely absent).
...
- **Why it matters / evidence:** For volunteer groups, where no one is paid to attend training, the lesson is that competence grows through how the group *works*, not through bolt-on sessions. If you want people to take decisions, normalize learning as part of the doing.
- **Related concepts:** learning-together [A6.6] (the engine of group effectiveness); competence-precondition [A9.2] (learning is how the precondition gets met); coaching-members [A9.6] (the leader grows others' competence).
- **Source reach:** Marquet's "competence" was heavily technical (reactor physics, navigation); for a volunteer group, read it as task-relevant skill and shared know-how. The principle — autonomy needs competence behind it — transfers; the curriculum doesn't.

### Don't brief, certify
concepts:    [A6.6 learning-together] [A9.7 onboarding-newcomers]
situations:  [S:newcomers-dont-stick]
handoff:
anchors to:  Divest control as competence and clarity rise
...
- **Why it matters / evidence:** For a volunteer leader, the everyday version is the difference between "does everyone understand the plan?" (met with nods) and "walk me through what you'll do." The second catches the misunderstanding that the first hides — especially with newcomers, who are most likely to nod and least likely to admit confusion.
- **Related concepts:** learning-together [A6.6] (certification is active learning); onboarding-newcomers [A9.7] (newcomers most need real checks, not nods); psychological-safety [A6.5] (it must be safe to reveal not-knowing).

### Deliberate action: errors aren't inevitable
concepts:    [A6.6 learning-together] [A6.5 psychological-safety]
situations:
handoff:
...
- **Why it matters / evidence:** For volunteer groups the literal practice may not transfer, but the principle does: errors are reducible through shared habits and a culture that treats them as information, and a leader who punishes honesty trains people to hide problems.
- **Related concepts:** learning-together [A6.6] (deliberate action is a competence practice); excellence-not-error-avoidance [A6.1] (punishing error breeds concealment); psychological-safety [A6.5] (rewarding candor over punishment).
- **Source reach:** This mechanism is rooted in a high-consequence physical-safety setting; for most volunteer groups it is thin and mostly illustrative. Keep it as a way to teach "errors are reducible, and how a leader reacts to them sets the culture," not as a procedure to install.

### Clarity makes decentralized decisions safe (the second precondition)
concepts:    [A2.1 shared-purpose] [A7.3 autonomy-vs-direction]
...
- **Why it matters / evidence:** This is the half of delegation that leaders forget. They divest the *what-to-do* but never establish the *what-we're-for*, and then blame people for poor judgment. Clarity is the precondition that turns autonomy into alignment rather than drift.
- **Related concepts:** shared-purpose [A2.1] (clarity is purpose made operational); autonomy-vs-direction [A7.3] (clarity is what makes autonomy safe); guiding-principles [A2.3] (the concrete form clarity takes).

### Guiding principles as decision criteria
concepts:    [A2.3 values-as-glue] [A2.1 shared-purpose]
...
- **Why it matters / evidence:** For volunteer groups that relitigate the same decisions endlessly, shared principles are the mechanism that lets a decision *stay* decided and lets new members decide like veterans. Co-authoring them is also what makes people actually use them.
- **Related concepts:** values-as-glue [A2.3] (principles are values made decision-usable); shared-purpose [A2.1] (principles operationalize the why); norms-and-operating-agreements [A5.5] (the how-we-decide layer — route execution detail to H:comms).
...
- **Why it matters / evidence:** When a volunteer leader notices "everyone just agrees with me," that's not a sign of good leadership — it's a risk. The fix is to make questioning the norm and to visibly thank the dissenter, especially when they turn out to be right.
- **Related concepts:** productive-dissent [A8.2] (questioning is dissent institutionalized); psychological-safety [A6.5] (it must be safe to contradict the leader); think-out-loud [A6.5] (voiced reasoning makes questioning natural).

### Achieve excellence, don't just avoid errors
concepts:    [A6.1 how-norms-form-and-become-culture] [A2.1 shared-purpose]
...
- **Why it matters / evidence:** Volunteer groups slide into error-avoidance easily, because mistakes feel personal and volunteers are easily discouraged. A leader who only ever flags what went wrong is, without meaning to, building a timid group. The reframe is toward what the group is *for*.
- **Related concepts:** how-norms-become-culture [A6.1] (what you measure becomes the culture); shared-purpose [A2.1] (excellence needs a positive why); psychological-safety [A6.5] (excellence cultures can hold honest error talk).

### Emancipation, not empowerment (why it all coheres)
concepts:    [A1.2 enabling-vs-doing] [A3.2 ownership-and-voice]
...
- **Why it matters / evidence:** This entry is where a coach ties Marquet together: the volunteer leader who feels they must do everything is, usually, still trying to *empower* (to grant, permit, enable) when the move is to *release*. Marquet's decade-later results — enduring performance and a flood of new leaders — are his evidence that emancipation, not empowerment, is what lasts.
- **Related concepts:** enabling-vs-doing [A1.2] (the leader's job is removing blocks, not granting power); ownership-and-voice [A3.2] (emancipated people own outcomes); the C-C-C triad [A9.2] (emancipation is the triad's payoff).
- **Source reach:** Marquet reaches emancipation from a position of formal power he chose to release. The volunteer leader reaches the same end state by refusing to centralize in the first place — the destination transfers even though the starting point is reversed.

---

## Layer 3 — Relationship Map

- **Divest control ↔ Control without competence** — [A9.2] × [A9.1]: divesting decisions is only safe once the decider is competent; the second is the precondition that governs the pace of the first. (Entries: "Divest control, keep responsibility", "Control without competence is chaos (the first precondition)")
- **Divest control ↔ Clarity makes decisions safe** — [A9.2] × [A2.1]: the more control is pushed out, the more each decider needs the shared purpose, or autonomy fragments. (Entries: "Divest control, keep responsibility", "Clarity makes decentralized decisions safe (the second precondition)")
- **"I intend to…" ↔ Resist the urge to provide solutions** — [A3.2] × [A1.2]: intent-language only shifts ownership if the leader answers the stated intention with a nod or a question, not an order. (Entries: "'I intend to…': language that moves ownership down", "Resist the urge to provide solutions")
- **Eliminate top-down monitoring ↔ Emancipation, not empowerment** — [A9.3] × [A1.2]: removing the tracking apparatus is emancipation in miniature — it stops the leader being the source of follow-through. (Entries: "Eliminate top-down monitoring", "Emancipation, not empowerment (why it all coheres)")
- **Specify goals, not methods ↔ Clarity makes decisions safe** — [A7.3] × [A2.1]: leaving the *how* to people only works if the *what* and the purpose behind it are unmistakably clear. (Entries: "Specify goals, not methods", "Clarity makes decentralized decisions safe (the second precondition)")
- **Guiding principles ↔ Questioning attitude** — [A2.3] × [A8.2]: shared principles give people the standing and the criteria to question a wrong call, including the leader's. (Entries: "Guiding principles as decision criteria", "Encourage a questioning attitude over blind obedience")
- **Achieve excellence ↔ Deliberate action** — [A6.1] × [A6.6]: an excellence culture can treat errors as information and design them out; an error-avoidance culture hides them. (Entries: "Achieve excellence, don't just avoid errors", "Deliberate action: errors aren't inevitable")
- **Think out loud ↔ Questioning attitude** — [A6.5] × [A8.2]: when the leader voices uncertainty aloud, it becomes safe for others to voice disagreement. (Entries: "Think out loud", "Encourage a questioning attitude over blind obedience")
- **When you step in, they step back ↔ "I intend to…"** — [A1.2] × [A3.2]: every leader takeover undoes the ownership that intent-language builds. (Entries: "When you step in, they step back", "'I intend to…': language that moves ownership down")
- **Don't brief, certify ↔ Control without competence** — [A6.6] × [A9.2]: certification is how a leader *verifies* the competence that licenses divesting control, instead of assuming it. (Entries: "Don't brief, certify", "Control without competence is chaos (the first precondition)")

---

## Flags

- **FLAG — register gap, no clean leaf (do not mint without sign-off):** Marquet's central contribution — *the precondition logic that divesting control requires competence and clarity built in parallel* — has no dedicated home in Axis A. It currently spreads across A9.2, A9.1, A2.1, and A7.3. Logged as a candidate for register review (a possible "preconditions for delegation" note rather than a new leaf). Per [D5], flagged not invented.
- **FLAG — register gap, cross-leaf concept:** "Emancipation vs empowerment" sits across A1.2 (enabling-vs-doing), A7.1 (building-and-using-power), and A3.2 (ownership-and-voice) with no single best home. Tagged primarily to A1.2 throughout; flagged for review.
- **FLAG — concrete practice under a broad leaf:** intent-based language ("I intend to…") is a specific, teachable practice living under A3.2 ownership-and-voice. Not a split candidate on its own evidence, but noted in case other sources surface adjacent "language/framing" practices that together warrant a leaf.
- **Authority inversion (carried in header + per-entry Source reach):** the whole Control pillar is written against the grain of the source's premise — Marquet divests held power; the volunteer leader avoids centralizing power they never held. Handled per [D1] with a header framing note and `Source reach:` lines, not a global caveat.
- **A10 material kept light (per [D3]):** take-care-of-your-people, immediate recognition, begin-with-the-end-in-mind, and legacy-for-inspiration are logged in Excluded rather than given entries; A10 is well-supplied elsewhere and Marquet's versions are Navy-flavored.

---

## Excluded from this summary

Cut as out-of-altitude, tactical, org-level, or already held at depth elsewhere in the corpus (logged for reviewability):

- **All submarine and Navy operational detail** — navigation, sonar, reactor operations, watch bills, weapons drills, ranks and the chain of command — kept only where it serves as a retell-ready illustration, never as subject matter.
- **Org-level program-design moves** — rewriting decision authorities across a 135-person command, force-wide certification programs, the "Chiefs in Charge" restructuring as an institutional reform. Read as illustrations of the leader's own-group moves; the program-architecture layer is `H:org-level`, not the volunteer leader's call.
- **The detailed mistake-critique process** — Marquet himself points readers off-book (to his website) for the seven-step critique procedure; that how-to is execution, not understanding. Kept only the principle (reward candor; design out recurrence).
- **Autonomy / mastery / purpose mechanics** — Marquet asserts intrinsic motivation but doesn't theorize it; deferred to **Pink, *Drive*** (A3.1), which holds this at depth. Marquet's entries cite it rather than re-explaining.
- **"Give the work back to the people" / adaptive vs. technical** — the deeper treatment of why leaders should not solve people's problems for them is held by **Heifetz & Linsky, *Leadership on the Line*** (A7.3/A1.2). Marquet's "resist the urge to provide solutions" is written as the in-the-moment behavioral complement, deferring the theory.
- **Structure-before-coaching, composition, launch** — held by **Fisher, *The Collective Edge*** (A5). Not re-opened here.
- **Culture as behavior and belonging cues** — "act your way to new thinking" overlaps **Coyle, *The Culture Code*** (A6.1/A6.3); kept only as the Layer 0 mechanism-over-personality principle, deferring the culture mechanics.
- **A10 cluster** — take-care-of-your-people (ch.33), immediate recognition (ch.36), begin-with-the-end-in-mind (ch.37), use-your-legacy-for-inspiration (ch.34): logged here per [D3]; available to revisit as light entries if the A10 domain proves thin after testing.
- **Short, early conversations / embrace the inspectors / continually repeat the message** — folded into Layer 0 (mechanisms-over-personality) and Layer 2 (certify; clarity) rather than given standalone entries, to avoid restating one idea as many.

---

*Provenance: Summarized from L. David Marquet, *Turn the Ship Around! A True Story of Turning Followers into Leaders* (Portfolio/Penguin, 2012; full text, ~66,490 words). Produced against the Group eCoach Tag Register (v2.0) and Master Working Reference (v2.0) under the four-layer source-summary format. All verbatim quotes verified against the source text; each under 15 words, one per entry maximum. Differentials per [D2]: overlapping concepts defer to Pink (Drive), Heifetz & Linsky (Leadership on the Line), Fisher (The Collective Edge), Coyle (The Culture Code). Decision markers D1–D6 approved by owner 2026-06-10. Sub-band length (~7.5K words ≈ ~11%) justified in the header sizing note.*

<!-- SEARCH_TERMS
marquet — turn the ship around | turn the ship around
domains: A1 | A6 | A7 | A9
leaves: A1.1 A1.2 A1.3 A1.5 A2.1 A2.3 A3.1 A3.2 A4.3 A5.5 A6.1 A6.5 A6.6 A7.1 A7.3 A7.6 A8.2 A8.3 A9.1 A9.2 A9.3 A9.4 A9.5 A9.6 A9.7
...
situations: cant-make-decisions decisions-keep-getting-relitigated doing-everything-myself everyone-just-agrees group-lost-its-why i-cant-let-go newcomers-dont-stick no-one-takes-initiative unclear-what-were-doing volunteers-flaky
-->
