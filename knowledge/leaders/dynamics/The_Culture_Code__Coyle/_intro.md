...
# _RAGSRC__ The Culture Code — Source Summary for the Group eCoach RAG

**Source:** Daniel Coyle, *The Culture Code: The Secrets of Highly Successful Groups* (Bantam, 2018)
...
> - The book is organized as three *skills*: **Build Safety** (ch.1–6), **Share Vulnerability** (ch.7–12), **Establish Purpose** (ch.13–17). The four-layer structure below maps those skills onto the register rather than following Coyle's tour order exactly.
> - **Overweighting watch.** Coyle fills the domains the corpus is already heavy in — A6 (dynamics/safety), A2 (purpose), A4 (trust). Amy Edmondson, intrinsic purpose, and "cohesion follows cooperation" all overlap Fisher / Pink / Ganz. Entries are trimmed to essentials and lean on Layer 3 cross-links rather than re-deriving shared ground.
> - **Constructs with no clean home (flagged, not forced).** Three of Coyle's central ideas have no dedicated leaf and are tagged to their nearest leaves: **belonging cues** (under A6.5 + A2.4 + A6.1), the **vulnerability loop** (under A4.3), and **catchphrases-as-heuristics** (under A6.1 + A5.5 + A2.3). Each is flagged inline.
...
The load-bearing claims the book rests on — the baselines a coach returns to across a whole class of problems. Each is individually retrievable and citable; Layer 2 entries point here by name via `anchors to:`.
