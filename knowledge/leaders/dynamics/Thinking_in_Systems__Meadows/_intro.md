...
# Thinking in Systems: A Primer — Source Summary (RAG)

**Product:** Group Leader eCoach · **Code:** (N0 corpus source) · **Owner:** Ned Howey · **Review with:** Instruction Set A (Master Working Reference) · **Version:** v1.0 · **Date:** 2026-06-10

> **Source:** Donella H. Meadows, *Thinking in Systems: A Primer* (Chelsea Green, 2008; written ~1993, published posthumously). ~67K words.
> **Provenance:** Summarized and tagged for the Group Leader eCoach RAG corpus per `master_prompt_source_summaries.md`. Tagged against Tag Register v2.0. Our structure, our words; short verified quotes only.

---

## Orientation: what this source is for, and how it was gated

Meadows is not an organizing book or a leadership book. It is a primer on how systems behave — written almost entirely at the scale of ecosystems, economies, populations, and corporations. **It earns its place in this corpus as a mental-models / diagnostic source, not a how-to.** Its contribution is *why structure produces behavior*: feedback loops, stocks, system traps, leverage points. That deepens the coach's core "why is this happening?" capacity — which is exactly the understanding coach's job — and supplies the theory under the register's existing **"structure before process"** lens.

**How the two gates were applied.** This source gates hard. Almost none of Meadows' *examples* are small-group; nearly all of her *abstractions* are scale-free and transfer. So the rule throughout was: **keep the abstraction, drop the example, re-ground it in a 3–7 person volunteer group.** The entire "Systems Zoo" of worked examples (thermostats, oil stocks, fishing fleets, capital-stock economies, population models), the macro/policy/ecosystem cases, the climate framing, and the equations/modeling appendix are logged in *Excluded* — the concepts they illustrate are kept, re-grounded.

> ⚠️ **MARK — register gap (decision needed; not resolved here).** Axis A has no home for systems-thinking machinery: *system, stock, flow, feedback loop, leverage point, resilience, self-organization*. These are cross-cutting **lenses**, not coachable concept-leaves (same shape as the Ganz strategy-domain gap). The *traps* map cleanly (mostly A6.4, A9.x, A10.x); the *foundational machinery* is homeless. Per protocol, every homeless concept below is tagged to its **nearest** existing leaf and flagged `register-gap` — **no codes invented.** Recommendation for your prune pass: add a third cross-cutting lens, **"systems thinking / structure produces behavior,"** sitting beside *structure-before-process* and *mobilizing-vs-organizing*. Do **not** add a Systems domain (these are lenses, not concept areas). Your call.

> ⚠️ **MARK — proportion.** This summary lands ~13% of source (~8,850 words), just below the 15–25% band. Deliberate: the in-scope fraction is small (massive gating — the entire Systems Zoo, the macro/policy/ecosystem cases, and the modeling appendix are cut), so padding those examples back in to hit a higher % would violate the gates. Depth-follows-source: the kept concepts are dense and full; the cut material is genuinely out of altitude.
...
The load-bearing claims the whole source rests on. A coach returns to these for a whole class of "why is my group doing this?" problems.
