...
# Thinking in Systems — ## Layer 2 — Concept Entries


Each entry stands alone as a retrievable chunk. Systems machinery without a register home is tagged to its nearest leaf and flagged `register-gap`.

### Your group is a system (look at the wiring, not the culprit)
concepts:    [A6.4 common-stall-patterns]   *(register-gap: the systems lens itself; nearest leaf used)*
...
- **Why it matters / evidence:** For a first-time leader this is the difference between churning through "unreliable" volunteers (and getting the same result with the next batch) and fixing the conditions that make reliability hard. Meadows' through-line: problems "most rooted in the internal structure of complex systems… refuse to go away" until you "see the system as the source of its own problems."
- **Related concepts:** bounded rationality [A3.2] (why the people are acting reasonably); common stall patterns [A6.4] (the recurring patterns themselves); leverage points [A5.5] (where to push once you see the structure).
- **Source reach:** Meadows argues this at ecosystem/economy scale; the small-group application is our translation. The principle is explicitly scale-free in the source, but the coach should attribute the *lens* to Meadows and the *group framing* to us.

### Stocks: the slow things you're really managing (trust, momentum, commitment)
concepts:    [A4.3 trust-building] [A6.3 cohesion-and-belonging] [A3.4 commitment-over-time]
...
- **Why it matters / evidence:** A leader fixated on inflow (recruit more!) while ignoring outflow (why are people drifting away?) keeps refilling a draining tub. Seeing trust and commitment as stocks reframes retention and cohesion as flow management, which is where the leverage usually is [A10.6].
- **Related concepts:** flows and delays [A6.2] (the rates that move the stock); cohesion-and-belonging [A6.3] (a stock that builds slowly through cooperation); intrinsic motivation [A3.2] (a major inflow to commitment).

### Flows, delays, and why change takes time
concepts:    [A6.2 stages-and-development] [A4.3 trust-building]
situations:  [S:lost-momentum-after-launch]
handoff:
...
- **Why it matters / evidence:** A first-time leader who launches a change, sees no movement in two weeks, and abandons it is reacting inside the delay — chasing their own tail. Knowing a stock has momentum lets a leader, in Meadows' image, use it like a judo expert uses an opponent's momentum: steer it rather than fight it.
- **Related concepts:** stocks [A4.3] (what the flows move); stages-and-development [A6.2] (group change unfolds on a delay); reinforcing loops [A9.3] (delays make loops oscillate).
- **Source reach:** Source examples are physical/economic (power plants, populations). The "don't over-correct, don't give up too soon" application to leading a group is our translation of a scale-free mechanism.

### Reinforcing loops: vicious and virtuous cycles
concepts:    [A9.3 avoiding-founder-bottleneck] [A6.4 common-stall-patterns]
...
- **Why it matters / evidence:** Most "my group is dying" and "my group is on fire" stories are reinforcing loops. Naming the loop tells a first-timer where to intervene — usually by changing one link (delegate an outcome, make a win visible) rather than by trying harder at everything [A9.2].
- **Related concepts:** balancing loops [A7.5] (the stabilizing counterpart); shifting the burden [A1.2] (the helpful-leader loop as a named trap); momentum-as-stock [A3.4] (what the loop is filling or draining).

### Balancing loops: why a group settles, and resists being moved
concepts:    [A6.4 common-stall-patterns] [A7.5 collective-agreement-tension]
...
- **Why it matters / evidence:** This is why willpower-based pushes fail and why relitigated decisions recur. For a first-timer it reframes resistance from "they're being difficult" to "the structure is defending a set point" — which points at agreement and shared goals, not pressure [A6.4].
- **Related concepts:** reinforcing loops [A9.3] (the amplifying counterpart); collective-agreement-tension [A7.5] (competing goals as the resistance); seeking the wrong goal [A5.3] (what set point the loop defends).

### Resilience: bounce-back comes from redundancy, not optimization
concepts:    [A10.5 resilience-through-setbacks] [A9.5 functional-shared-leadership]
...
- **Why it matters / evidence:** A first-time leader is tempted to be the single reliable point through which everything runs — maximally efficient, minimally resilient. One illness, one burnout, one departure and the group crashes [A9.5]. Resilience is the case for distributing capacity *before* you need it.
- **Related concepts:** self-organization [A7.3] (the deeper capacity redundancy protects); shifting the burden [A9.3] (how over-help erodes resilience); collective-care [A10.4] (redundancy as a care practice).

### Self-organization: the group's capacity to grow itself — and how control kills it
concepts:    [A7.3 autonomy-vs-direction] [A3.2 ownership-and-voice] [A9.1 developing-leaders]
...
- **Why it matters / evidence:** "No one takes initiative" and "I can't let go" are usually the same problem seen from two sides — a leader controlling tightly enough that self-organization has no room. Loosening specific controls (handing over an outcome, inviting members to shape the how) is how a first-timer turns passive members into co-leaders [A9.1].
- **Related concepts:** autonomy-vs-direction [A7.3] (direct the what, free the how); ownership-and-voice [A3.2] (people commit to what they help shape); self-organization as leverage [A2.1] (one of Meadows' highest leverage points).
- **Source reach:** Meadows treats self-organization across biology, technology, and culture. The "don't smother your group's self-organization" application is our re-grounding of a claim she makes at every scale.

### Just-enough hierarchy: structure serves the small units, not the reverse
concepts:    [A7.6 hierarchy-just-enough] [A5.2 group-size-and-nesting]
...
- **Why it matters / evidence:** A first-time leader of a growing group reaches for more rules and more reporting. Meadows' frame says: add the *minimum* coordinating structure, and make sure it serves the sub-teams. Anything more starves the self-organization that makes the group work [A7.3].
- **Related concepts:** group-size-and-nesting [A5.2] (the structural move when a circle gets too big); self-organization [A7.3] (what over-control destroys); hierarchy-just-enough [A7.6] (the calibration itself).
- **Source reach:** *org-check / H:org-level* — designing cross-group structure is the org's call, not the volunteer leader's; keep this at the why-level (why minimal hierarchy) and route program-design questions out.

### Bounded rationality: the "flaky" volunteer is being reasonable
concepts:    [A8.1 task-vs-relationship-conflict] [A3.2 ownership-and-voice] [A6.4 common-stall-patterns]
...
- **Why it matters / evidence:** This is the single most useful reframe in the book for a first-time leader. It converts "my volunteers are unreliable" (a dead end) into "what does commitment look like from where they sit, and what would change it?" — and it keeps conflict on the situation, not the person [A8.1]. Meadows is explicit this is *not* an excuse for bad behavior; it's an explanation that points at a fix.
- **Related concepts:** information flows as leverage [A5.5] (changing what people see); your group is a system [A6.4] (the structure produces the behavior); task-vs-relationship conflict [A8.1] (issue, not person).

### Look below the event: events, behavior, structure
concepts:    [A6.4 common-stall-patterns]   *(register-gap: levels-of-seeing; nearest leaf used)*
...
- **Why it matters / evidence:** First-time leaders over-react to single meetings — a great one breeds complacency, a bad one breeds panic. Training the eye to ask "what's the trend, and what structure produces it?" is the move from firefighting to leading. It is also the coach's own diagnostic discipline.
- **Related concepts:** your group is a system [A6.4] (structure as cause); dancing with systems [A6.6] (watch behavior before theorizing); leverage points [A5.5] (intervene at structure, not event).

### Trap — Shifting the burden: the over-helpful leader makes the group dependent
concepts:    [A9.3 avoiding-founder-bottleneck] [A1.2 enabling-vs-doing] [A9.2 delegating-outcomes-not-tasks]
...
- **Why it matters / evidence:** This is the systems mechanism beneath founder burnout and the founder bottleneck — and it is precisely what Tectonica's *Friction by Design* guards against: designed friction keeps the group doing its own work rather than offloading it onto the leader (or the tool). The way out is to take less, not more, and to invest the freed effort in the group's capacity [A1.2].
- **Related concepts:** reinforcing loops [A9.3] (the deepening cycle); enabling-vs-doing [A1.2] (the leader's role shift); delegating-outcomes-not-tasks [A9.2] (rebuilding capacity).

### Trap — Tragedy of the commons: leaning on the reliable few until they break
concepts:    [A10.1 volunteer-burnout] [A8.3 peer-accountability-without-authority] [A5.4 work-and-task-design]
...
- **Why it matters / evidence:** "All the work is on a few people" is a commons in disguise: relying on the dependable is each member's locally rational default, and it erodes the very people the group can't lose [A10.1]. The durable fix is shared agreement that distributes the load and makes over-leaning visible — not pleading with people to volunteer more.
- **Related concepts:** peer-accountability-without-authority [A8.3] (the "mutually agreed upon" part); volunteer-burnout [A10.1] (the eroded resource); success to the successful [A9.2] (the related "star gets more" trap).

### Trap — Drift to low performance: standards that quietly slide
concepts:    [A6.4 common-stall-patterns] [A3.4 commitment-over-time] [A5.3 goal-design]
...
- **Why it matters / evidence:** Post-launch slumps often aren't a motivation problem; they're an eroding-goals problem the leader didn't notice giving ground. For a first-timer, the move is to make the standard explicit and stable before disappointment quietly renegotiates it downward [A5.3].
- **Related concepts:** balancing loops [A6.4] (the goal-defending mechanism); goal-design [A5.3] (clear, vivid, stable goals resist drift); commitment-over-time [A3.4] (what erodes alongside the standard).

### Trap — Success to the successful: always handing the plum work to the star
concepts:    [A9.1 developing-leaders] [A9.2 delegating-outcomes-not-tasks] [A5.4 work-and-task-design]
...
- **Why it matters / evidence:** This is why "no one else steps up" can coexist with "I have one amazing volunteer." The structure concentrates capability by rewarding it, starving the development of everyone else [A9.2]. For a leader trying to distribute leadership, recognizing the trap is the prompt to hand the next stretch task to a newer member.
- **Related concepts:** developing-leaders [A9.1] (the capacity being starved); tragedy of the commons [A10.1] (the related over-leaning trap); delegating-outcomes-not-tasks [A9.2] (the redistribution move).

### Trap — Seeking the wrong goal (and rule-beating): chasing the metric, missing the mission
concepts:    [A5.3 goal-design] [A2.1 shared-purpose / why-before-what] [A5.5 norms-and-operating-agreements]
...
- **Why it matters / evidence:** "We lost our why" often starts as a quietly mis-set goal: the group began steering by an easy metric and drifted from its purpose [A2.1]. For a first-timer the discipline is to keep the goal pinned to purpose and to notice when a well-followed rule is producing something no one actually wants.
- **Related concepts:** goal-design [A5.3] (set the goal as purpose); shared-purpose [A2.1] (the why the metric should serve); leverage points [A6.1] (goals are high-leverage).

> *Note: the escalation trap (arms-race / competitive one-upping) is logged in Excluded — a thin fit for a 3–7 person group. If conflict between two members spirals competitively, the relevant home is A8.1 / A8.4, not this archetype.*

### The leverage ladder: stop pushing the weak places
concepts:    [A2.1 shared-purpose / why-before-what] [A5.3 goal-design] [A6.1 how-norms-form-and-become-culture]
situations:  [S:lost-momentum-after-launch]
handoff:
anchors to:  Intervene high, not low
...
- **Why it matters / evidence:** A first-time leader's instinct under stress is to push parameters harder — more emails, more reminders, more pleading. The ladder tells them that effort is nearly wasted and points them at norms, information, and purpose instead [A6.1]. It is the practical companion to "structure produces behavior": this is *where* to change the structure.
- **Related concepts:** information flows as leverage [A5.5] (a high rung, often missing feedback); goals and paradigm [A2.1] (the highest practical rungs); your group is a system [A6.4] (why low-leverage pushes fail).
- **Source reach:** The full 12-rung ladder is Meadows' famous schema, developed for policy and economic systems; for the coach the usable core is the *direction* (low = numbers/effort; high = information, rules, goals, mindset), not the exact ordinal positions, which she herself calls provisional.

### Information flows as leverage: change what people can see
concepts:    [A5.5 norms-and-operating-agreements] [A6.6 learning-together] [A7.2 decisions-as-shared-process]
...
- **Why it matters / evidence:** A first-time leader facing "no one follows through" often has an information problem, not a character problem: people can't see what's needed, who's counting on them, or what happened last time [A6.6]. Making contributions and consequences visible (a shared tracker, a quick debrief) is frequently the lever — the one-third effect, re-grounded.
- **Related concepts:** bounded rationality [A3.2] (behavior follows visible information); learning-together [A6.6] (feedback as the group's learning loop); the leverage ladder [A5.5] (information flows sit high on it).

### Goals and paradigm: the highest leverage is the why and the self-image
concepts:    [A2.1 shared-purpose / why-before-what] [A6.1 how-norms-form-and-become-culture] [A2.4 group-identity-and-belonging]
...
- **Why it matters / evidence:** "We lost our why" and "it all feels transactional" are paradigm/goal problems, and a first-time leader who keeps adjusting tactics (the low rungs) won't touch them. Re-grounding the group's purpose and felt identity is the high-leverage repair — and exactly the transformational-vs-transactional lens [A2.5] in systems terms.
- **Related concepts:** shared-purpose [A2.1] (the goal rung); how-norms-form-and-become-culture [A6.1] (paradigm becoming culture); seeking the wrong goal [A5.3] (what happens when the goal is mis-set).

### Dancing with systems: the coaching posture (get the beat before you intervene)
concepts:    [A1.6 failure-as-learning] [A6.6 learning-together] [A7.2 decisions-as-shared-process]
...
- **Why it matters / evidence:** This is the systems backbone of a Socratic, friction-by-design coaching posture: build the group's judgment rather than substitute the leader's, watch before prescribing, and let members feel the results of their own calls [A6.6]. For a first-timer it licenses *not* having the answer immediately — observing the pattern is the work, not a delay before the work.
- **Related concepts:** look below the event [A6.4] (watch behavior over time); self-organization [A7.3] (what humility leaves room for); failure-as-learning [A1.6] (surprises as information).
- **Source reach:** Meadows offers ~14 "dancing" guidelines; this entry distils the four most transferable to a small-group leader (watch first, stay in learning mode, don't define the problem as your missing solution, locate responsibility in the structure). The rest sit at analyst/society scale and are logged in Excluded.

---


<!-- SEARCH_TERMS
meadows — thinking in systems | thinking in systems
domains: A5 | A6 | A9
leaves: A1.2 A1.6 A10.1 A10.4 A10.5 A10.6 A2.1 A2.4 A2.5 A3.2 A3.4 A4.3 A5.1 A5.2 A5.3 A5.4 A5.5 A6.1 A6.2 A6.3 A6.4 A6.6 A7.2 A7.3 A7.5 A7.6 A8.1 A8.3 A9.1 A9.2 A9.3 A9.5
...
situations: burning-out cant-make-decisions conflict-got-personal decisions-keep-getting-relitigated doing-everything-myself group-feels-transactional group-lost-its-why group-too-big-or-chaotic i-cant-let-go lost-momentum-after-launch low-attendance no-one-takes-initiative volunteers-flaky work-is-all-on-a-few-people
-->