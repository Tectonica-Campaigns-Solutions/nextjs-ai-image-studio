...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (imageFiles.length > 0) {
      ...
      try {
        let restoredBuffer: Buffer | null = NON_DESTRUCTIVE_DISCLAIMER
          ? await fetchCleanInputBuffer(imageUrls[0])
          : null
        if (restoredBuffer) {
    ...
    try {
      ...
      // Non-destructive pipeline: when a disclaimer is due, store the clean
      // buffer and let the proxy compose the overlay on every read instead of
      // baking it in now (legacy fallback when the flag is off).
