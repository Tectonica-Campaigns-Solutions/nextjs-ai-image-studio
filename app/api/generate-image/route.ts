...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (referenceImage) {
        console.log("[v0] Processing reference image, name:", referenceImage.name, "size:", referenceImage.size);
      ...
      if (result.data && result.data.images && result.data.images.length > 0) {
        ...
        return NextResponse.json({ error: "Unexpected output format from FLUX Kontext" }, { status: 500 });
      }
    } catch (error) {
      ...
      return NextResponse.json({ error: `FLUX Kontext processing failed: ${error}` }, { status: 500 });
    }
  } catch (error) {
