...
import type { CreateClientInput } from "@/app/(studio)/dashboard/features/clients/schemas/clients";
...
import { softDeletePayload } from "@/app/(studio)/dashboard/utils/action-utils";
...
export async function deleteClientAction(
  ...
  const { error } = await supabase
    .from("clients")
    .update(softDeletePayload(check.user.id))
    .eq("id", clientId);
